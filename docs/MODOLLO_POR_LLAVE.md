...
- **Creación de JSON y árbol BC3:** el backend usa **automáticamente** la llave que tenga el modelo `groq/compound` (muchos tokens por respuesta, 70K TPM). Si no hay ninguna con compuesto, se usa la llave indicada en `GROQ_LLAVE_SOLO_BC3`. Esa llave solo se usa para BC3; el resto de peticiones usan las otras llaves.
...
No hace falta poner `GROQ_LLAVE_SOLO_BC3`: si alguna llave tiene `groq/compound`, el backend ya usa esa llave solo para creación de JSON/árbol.
